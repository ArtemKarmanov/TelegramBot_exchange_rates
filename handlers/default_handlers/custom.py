	...
	if action == "DAY":
		...
		# Если состояние пользователя на конечной дате
			...
			with bot.retrieve_data(call.from_user.id, call.message.chat.id) as data:
				...
				end_date = datetime.datetime.strptime(data['start_date'], '%d.%m.%Y')
				max_days = datetime.timedelta(days=365)

				# Конечная не может быть больше начальной
				if start_date < end_date:
					bot.send_message(
						chat_id=call.message.chat.id,
						text=f'Конечная дата должна быть раньше начальной! Вы указали {set_date}. Попробуйте снова.'
					)
